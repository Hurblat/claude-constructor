...
- If there are no directories using the format `prompt-{number}`, use issue key `prompt-1`
- If there is at least one directory using the format `prompt-{number}`, use issue key `prompt-{number+1}`
